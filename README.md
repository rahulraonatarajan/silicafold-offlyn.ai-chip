...
**Status: Pending verification with updated workflow**

The GDS hardening workflow uses the official Tiny Tapeout SKY130 action (`@ttsky26c`). When the workflow passes, the following artifacts will be available:

| Artifact | Contents |
|----------|----------|
| GDS file | Final layout for fabrication |
| Reports | Synthesis, timing, utilization, DRC, LVS |
| Precheck | Tiny Tapeout submission validation |

Download artifacts from [GitHub Actions](https://github.com/rahulraonatarajan/silicafold-offlyn.ai-chip/actions).

### Concept Image vs Actual GDS

Any architecture diagram in this README is a **conceptual visualization** unless explicitly labeled as generated from the Tiny Tapeout/OpenLane flow.

Do not treat conceptual images as:
- Final GDS output
- OpenLane/OpenROAD results
- DRC/LVS evidence
- Measured silicon

Actual physical-design artifacts must come from the GDS workflow.
...
**Current status: Pre-submission hardening**

The design should not be submitted until all of the following are true:
...
| Timing report available | Check Actions |
| DRC report available | Check Actions |
| LVS report available | Check Actions |
| No severe Yosys warnings | Review logs |
| info.yaml validated | ✅ Matches template |
| Template alignment | ✅ Uses @ttsky26c |

See [docs/current_submission_status.md](docs/current_submission_status.md) for detailed tracking.
