...
The following image shows the **actual physical layout** generated by the Tiny Tapeout/OpenLane flow:

![SilicaFold V0 GDS Layout](assets/gds_render.png)

### What You're Seeing

This is the real GDS (Graphic Data System) output from the OpenLane ASIC synthesis flow:

| Layer | Color | Description |
|-------|-------|-------------|
| **Blue/Cyan areas** | Metal routing layers | Interconnect wires connecting logic cells |
| **Red/Pink areas** | Polysilicon | Transistor gates |
| **Green areas** | Active diffusion | Transistor source/drain regions |
| **Regular grid** | Standard cells | Pre-characterized logic gates (AND, OR, flip-flops, etc.) |
| **Boundary** | Die outline | 4x2 Tiny Tapeout tile allocation (682.64 × 225.76 µm) |

### Design Metrics

The hardening flow completed successfully with:

- **DRC**: Clean (Magic + KLayout)
- **LVS**: Clean (Netgen)
- **Timing**: No setup/hold violations at 25 MHz
- **Slew/Cap**: No violations

### Artifacts Available

Download from [GitHub Actions](https://github.com/rahulraonatarajan/silicafold-offlyn.ai-chip/actions):

| Artifact | Contents |
|----------|----------|
| `gds` | Final GDS file (`tt_um_rahulraonatarajan_silicafold_v0.gds`) |
| `gds_render` | PNG render of the layout |
| `reports` | Synthesis, timing, utilization reports |
| `openlane-run` | Complete OpenLane run directory |
| `tt_submission` | Tiny Tapeout submission package |

## Concept Image vs Actual GDS

The architecture block diagram at the top of this README is a **conceptual visualization** showing the logical organization. The GDS render above shows the **actual physical implementation** after synthesis, placement, and routing.

Key differences:
- Concept diagrams show logical blocks and data flow
- GDS shows actual transistors, metal layers, and physical routing
- Concept diagrams are for understanding; GDS is for fabrication
